...
`just setup` links Streamlit’s skill into `.cursor/skills/developing-with-streamlit` (from `.venv`). **Tidy First** skills are committed under `.cursor/skills/` and `.agents/skills/`: `tidy-first` (inline while coding) and `tidy-project` (whole-repo tidy PRs). In **Agent** chat at the repo root, type **`/tidy-project`** (from `.cursor/commands/tidy-project.md`) or pick the skill under **Customize → Skills** — see [AGENTS.md](AGENTS.md) if it does not appear.
