...
def testload_week_plan_names_falls_back_to_legacy_path(tmp_path: Path, monkeypatch) -> None:
    ...
    assert load_week_plan_names(WEEK_PLAN_PATH) == ["Legacy Soup"]


def testload_week_plan_names_invalid_json(
    ...
    assert load_week_plan_names(path) == []
    assert "could not read week plan" in capsys.readouterr().err


def testload_week_plan_names_oserror_returns_empty(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Patch read_text to raise OSError and assert load_week_plan_names returns [] gracefully."""
    ...
    with patch.object(_Path, "read_text", side_effect=OSError("permission denied")):
        result = load_week_plan_names(path)
